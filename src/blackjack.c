...
void handle_click(struct Game *game, SDL_MouseButtonEvent *button, int mouse_x, int mouse_y) {
  ...
  switch (game->current_menu) {
    ...
    case MenuDeckType:
      if (inside_rect(game->btn_rects[BtnRegular], mouse_x, mouse_y)) {
        new_regular(game);
        game->current_menu = MenuHand;
        deal_new_hand(game);
        return;
      }
      if (inside_rect(game->btn_rects[BtnAces], mouse_x, mouse_y)) {
        new_aces(game);
        game->current_menu = MenuHand;
        deal_new_hand(game);
        return;
      }
      if (inside_rect(game->btn_rects[BtnJacks], mouse_x, mouse_y)) {
        new_jacks(game);
        game->current_menu = MenuHand;
        deal_new_hand(game);
        return;
      }
      if (inside_rect(game->btn_rects[BtnAcesJacks], mouse_x, mouse_y)) {
        new_aces_jacks(game);
        game->current_menu = MenuHand;
        deal_new_hand(game);
        return;
      }
      if (inside_rect(game->btn_rects[BtnSevens], mouse_x, mouse_y)) {
        new_sevens(game);
        game->current_menu = MenuHand;
        deal_new_hand(game);
        return;
      }
      if (inside_rect(game->btn_rects[BtnEights], mouse_x, mouse_y)) {
        new_eights(game);
        game->current_menu = MenuHand;
        deal_new_hand(game);
        return;
    ...
    case MenuInsurance:
      if (inside_rect(game->btn_rects[BtnInsureYes], mouse_x, mouse_y)) {
        insure_hand(game);
        return;
      }
      if (inside_rect(game->btn_rects[BtnInsureNo], mouse_x, mouse_y)) {
        no_insurance(game);
        return;
      }
      break;
...
void load_btn_textures(struct Game *game) {
  ...
  if (btn_insure_yes_surface == NULL) {
    printf("Unable to load image %s! SDL Error: %s\n", "img/btn_insure_yes.bmp", SDL_GetError());
    exit(EXIT_FAILURE);
  }
  if (btn_insure_no_surface == NULL) {
    printf("Unable to load image %s! SDL Error: %s\n", "img/btn_insure_no.bmp", SDL_GetError());
    exit(EXIT_FAILURE);
  }
...
void draw_dealer_hand(const struct Game *game) {
  ...
  for (unsigned i = 0; i < dealer_hand->hand.num_cards; ++i) {
    ...
    if (i == 0) {
      ...
      write_text(game, str, FontMd, (int) (x_offset + hand_width(dealer_hand->hand.num_cards) - (unsigned) count_w - 3),
                 (int) DEALER_HAND_Y_OFFSET + CARD_H + 2);
    }
  }
}
...
void draw_player_hands(const struct Game *game) {
  ...
  int count_w, count_h;
  char str[32], plus_minus[2], current[3];
  ...
  for (h = 0; h < game->total_player_hands; h++) {
    ...
    for (c = 0; c < player_hand->hand.num_cards; c++) {
      ...
      if (c == 0) {
        ...
        TTF_SizeText(game->fonts[FontMd], str, &count_w, &count_h);
        write_text(game, str, FontMd, (int) (x + hand_width(player_hand->hand.num_cards) - (unsigned) count_w - 3),
                   (int) game->player_hands_y_offset + CARD_H + 2);
      }
